  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center" style={{ background: '#F0F4FF', fontFamily: "'Poppins', sans-serif" }}>
      ...
      <div className="relative">
        ...
        <h1 className="font-bold text-2xl mb-1" style={{ color: '#0F172A' }}>Pesanan Dibuat! 🎉</h1>
        <p className="mb-1" style={{ color: '#94A3B8' }}>Perlengkapan badminton kamu sedang dalam perjalanan</p>
        ...
        <div className="rounded-3xl p-5 mb-6 text-left w-full max-w-sm mx-auto bg-white" style={{ border: '1px solid #E2E8F0', boxShadow: '0 4px 20px rgba(0,0,0,0.06)' }}>
          <div className="flex justify-between mb-3">
            <span className="text-xs" style={{ color: '#94A3B8' }}>ID Pesanan</span>
            <span className="text-xs font-semibold" style={{ color: '#0F172A' }}>{orderId}</span>
          </div>
          <div className="flex justify-between mb-3">
            <span className="text-xs" style={{ color: '#94A3B8' }}>Total Pembayaran</span>
            <span className="text-sm font-bold" style={{ color: '#1D4ED8' }}>{formatCurrency(total)}</span>
          </div>
          <div className="flex justify-between mb-3">
            <span className="text-xs" style={{ color: '#94A3B8' }}>Estimasi Pengiriman</span>
            <span className="text-xs font-medium" style={{ color: '#0F172A' }}>2 - 4 Hari Kerja</span>
          </div>
          <div className="flex justify-between">
            <span className="text-xs" style={{ color: '#94A3B8' }}>Poin Didapatkan</span>
          ...
          {typeof currentPoints === 'number' && (
            <div className="flex justify-between mt-1">
              <span className="text-xs" style={{ color: '#94A3B8' }}>Saldo Poin</span>
          ...
          <div className="mt-4 pt-4 border-t" style={{ borderColor: '#F1F5F9' }}>
            <div className="flex items-center gap-2 p-3 rounded-xl" style={{ background: 'rgba(29,78,216,0.06)', border: '1px solid rgba(29,78,216,0.12)' }}>
              ...
              <span className="text-xs" style={{ color: '#1D4ED8' }}>Nomor resi akan dikirimkan ke email kamu</span>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-sm mx-auto mb-8">
          {[
            ...
            { label: 'Diproses', done: false, active: true },
            { label: 'Dikirim', done: false, active: false },
            { label: 'Terkirim', done: false, active: false },
          ].map((s, i, arr) => (
            <div key={s.label} className="flex items-start gap-3 mb-3 last:mb-0">
              ...
              <div className="pt-0.5">
                ...
                {s.active && <p className="text-[10px]" style={{ color: '#94A3B8' }}>Pesanan kamu sedang dipersiapkan</p>}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-3 w-full max-w-sm mx-auto">
          ...
          <button onClick={() => navigate('/')}
            ...
            <Home size={15} /> Lanjut <ChevronRight size={14} />
          </button>
        </div>
        ...
        <div className="mt-4 p-3 rounded-2xl flex items-center gap-3 w-full max-w-sm mx-auto bg-white" style={{ border: '1px dashed #CBD5E1' }}>
          ...
          <p className="text-xs" style={{ color: '#94A3B8' }}>Setelah menerima pesanan, beri ulasan dan dapatkan poin bonus!</p>
        </div>
      </div>
    </div>
  );
