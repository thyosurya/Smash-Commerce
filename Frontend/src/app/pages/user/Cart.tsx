    ...
    <div className="min-h-screen" style={{ background: '#F0F4FF', fontFamily: "'Poppins', sans-serif" }}>
      ...
      <div className="sticky top-0 z-30 px-4 pt-10 pb-4" style={{ background: '#FFFFFF', borderBottom: '1px solid #E2E8F0', boxShadow: '0 1px 6px rgba(0,0,0,0.05)' }}>
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-bold text-lg" style={{ color: '#0F172A' }}>Keranjang Saya</h1>
          {cart.length > 0 && (
      ...
      {cart.length === 0 ? (  
        <div className="flex flex-col items-center justify-center py-20 px-6 text-center">
          ...
          <h2 className="font-semibold text-lg mb-1" style={{ color: '#0F172A' }}>Keranjang kamu kosong</h2>
          <p className="text-sm mb-6" style={{ color: '#94A3B8' }}>Tambahkan perlengkapan badminton untuk mulai belanja</p>
          <button onClick={() => navigate('/')}
            className="flex items-center gap-2 px-6 py-3 rounded-xl text-white font-medium text-sm"
            style={{ background: 'linear-gradient(135deg, #1D4ED8, #2563EB)' }}>
            Mulai Belanja <ChevronRight size={16} />
          </button>
        </div>
      ) : (
        <div className="p-4">
          ...
          <div className="space-y-3 mb-4">
            {cart.map(item => (
              <div key={item.product.id} className="rounded-2xl p-3.5 flex gap-3 bg-white" style={{ border: '1px solid #E2E8F0', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
                ...
                <button onClick={() => { removeFromCart(item.product.id); toast.success('Item dihapus'); }}
          ...
          <div className="rounded-2xl p-4 mb-4 space-y-2.5 bg-white" style={{ border: '1px solid #E2E8F0', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
            <h3 className="font-semibold text-sm mb-3" style={{ color: '#0F172A' }}>Ringkasan Pesanan</h3>
            <div className="flex justify-between text-sm">
              <span style={{ color: '#94A3B8' }}>Subtotal ({cart.length} produk)</span>
            ...
            {discount > 0 && (
              <div className="flex justify-between text-sm">
                <span style={{ color: '#10B981' }}>Diskon Loyalitas (5%)</span>
            ...
            <div className="pt-2 border-t flex justify-between" style={{ borderColor: '#F1F5F9' }}>
              <span className="font-semibold" style={{ color: '#0F172A' }}>Total Bayar</span>
          ...
          <button onClick={() => {
            ...
            Lanjut ke Checkout <ChevronRight size={16} />
          </button>
